...
#%% Read in datafile
with open('board3_data.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow("ave0,ave1,ave2,med0,med1,med2".split(","))

all_avg = []
all_med = []
for i in range(3):
    df = pd.read_csv(f"./sensor_data/board/matrix_data_board{i}.csv")
    print(df.head())
    ...
    for x in range(len(sensor_array)):
        ...
        peak,_ = find_peaks(peak_data, distance = 125, prominence = 10)
        valley_data = peak_data*(-1)
        valley,_ = find_peaks(valley_data,distance = 125, prominence = 10)
    ...
    all_avg.append(ave)
    all_med.append(median_val)

# transpose
all_avg = np.array(all_avg).T
all_med = np.array(all_med).T
# write to csv file
with open('board3_data.csv', 'a', newline='') as file:
    writer = csv.writer(file)
    for i in range(len(all_avg)):
        writer.writerow(all_avg[i] + all_med[i])
